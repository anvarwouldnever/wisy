...
const Game2Screen = ({ data, setLevel }) => {
    ...
    return (
        <View style={{position: 'absolute', top: 24, width: windowWidth - 60, height: windowHeight - 60}}>
            {data && <Game2Animals1Animation text={text} answer={answer} images={data.content.images} animal={data.content.title}/>}
            <View style={{width: 'auto', height: 'auto', alignItems: 'center', flexDirection: 'row', justifyContent: 'space-between', position: 'absolute', bottom: 0, left: 0}}>
                <View style={{width: windowWidth * (255 / 800), height: Platform.isPad? windowWidth * (200 / 800) : windowHeight * (80 / 360), alignSelf: 'flex-end', alignItems: 'flex-end', flexDirection: 'row'}}>
