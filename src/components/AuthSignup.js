    ...
    return (
        <Animated.View style={{justifyContent: 'space-between', marginTop: 30, alignSelf: 'center', width: width * 0.8666, height: height * 0.7725}}>
                ...
                <View behavior='height' style={{alignItems: 'center', flexDirection: 'column', justifyContent: 'space-between', width: width * 0.8666, height: 'auto', gap: height * (12 / 800)}}>
                    <TextInput onChangeText={(text) => setEmail(text)} keyboardType='email-address' style={{fontWeight: '600', fontSize: height * (14 / 800), padding: height * (16 / 800), borderRadius: 100, borderWidth: 1, borderColor: error === 'email'? '#D83636' : '#E5E5E5', borderStyle: 'solid', width: width * 0.8666, height: height * (56 / 800)}} placeholderTextColor={'#B1B1B1'} placeholder="Email"/>
                        {error === 'email' 
                        && 
                ...
                <TouchableOpacity onPress={email === ''? () => setError('email') : password === ''? () => setError('password') : () => signUp()} style={{justifyContent: 'center', alignItems: 'center', opacity: email != '' && password != ''? 1 : 0.5, alignSelf: 'center', width: width * 0.8666, height: height * (56 / 800), backgroundColor: '#504297', borderRadius: 100, }}>
                ...
                <View style={{justifyContent: 'flex-end', width: width * 0.8666, height: height * (97 / 800)}}>
                    <View style={{flexDirection: 'row', justifyContent: 'center', alignItems: 'center', width: width * 0.8666, height: height * (20 / 800)}}>
                        <Text style={{color: '#555555', fontWeight: '600', fontSize: height * (12 / 800)}}>I already have an account.</Text>
                        <TouchableOpacity onPress={() => toggleOption('login')} style={{marginLeft: 2}}>
                            <Text style={{color: '#504297', fontWeight: '600', fontSize: height * (12 / 800)}}>Log in</Text>
                        </TouchableOpacity>
                    </View>
                </View>
